...
def aggregate_jurisdiction(rows):
    ...
    total_pop = sum(r["b02001_001e"] for r in rows)
    ...
    avg_nwi = weighted_sum / total_pop if total_pop > 0 else 0
...
def main():
    ...
    # Cities — key by city_name + state FIPS
    print("Aggregating cities...")
    cities = build_level(
        records,
        key_fn=lambda r: f"{r.get('city_name', '')}|{r['geoid10'][:2]}" if r.get("city_name") else None,
        name_fn=lambda r: f"{r['city_name']}, {r['state_name']}",
    )
